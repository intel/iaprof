...
sub color {
	...
	# multi palettes
	if (defined $type and $type eq "gpu") {
		...
		if ($name =~ m:_\[g\]$:) {	# GPU
			$type = "green";
		} elsif ($name =~ m:_\[G\]$:) {	# GPU symbol
			$type = "aqua";
		} elsif ($name =~ m:^NEOxx:) {	# GPU library
			$type = "aqua";
...
while (my ($id, $node) = each %Node) {
	...
	if ($func eq "" and $depth == 0) {
		...
		$escaped_func =~ s/>/&gt;/g;
		$escaped_func =~ s/"/&quot;/g;
		$escaped_func =~ s/_\[[kwijgG]\]$//;	# strip any annotation
	...
	if ($chars >= 3) { # room for one char plus two dots
		$func =~ s/_\[[kwijgG]\]$//;	# strip any annotation
