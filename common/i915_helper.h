...
void *i915_gem_mmap_offset(int fd, uint32_t handle, uint64_t offset, uint64_t size, unsigned int prot, uint64_t flags) {
  struct drm_i915_gem_mmap_offset arg;
  void *ptr;
...
void *i915_gem_mmap_offset_cpu(int fd, uint32_t handle, uint64_t offset, uint64_t size, unsigned prot) {
  void *ptr;

  ptr = i915_gem_mmap_offset(fd, handle, offset, size, prot, I915_MMAP_OFFSET_WB);
  if (!ptr) {
...
void *i915_gem_mmap_cpu_coherent(int fd, uint32_t handle, uint64_t offset, uint64_t size, unsigned prot) {
...
void i915_gem_sync(int fd, uint32_t handle)
{
  if (i915_gem_wait(fd, handle, NULL))
    i915_gem_set_domain(fd, handle,
             I915_GEM_DOMAIN_GTT,
             I915_GEM_DOMAIN_GTT);
  errno = 0;
}

int i915_gem_write(int fd, uint32_t handle, uint64_t offset, const void *buf, uint64_t length) {
  void *map = NULL;
...
struct buff *i915_buff_create(uint32_t handle, int width, int height, int bpp, int alignment) {
  uint64_t size;
  struct buff *buf;
...
struct drm_i915_gem_exec_object2 *i915_batchbuf_add_to_cache(struct batchbuffer *bb, uint32_t handle) {
...
struct drm_i915_gem_exec_object2 *i915_batchbuf_add_object(struct batchbuffer *bb, uint32_t handle, uint64_t size, uint64_t offset, uint64_t alignment, bool write) {
  ...
  if(bb->num_objects <= bb->num_alloc_objects) {
    ...
    bb->objects = realloc(bb->objects, bb->num_alloc_objects * sizeof(struct drm_i915_gem_exec_object2 *));
    memset(&(bb->objects[prev_num_alloc_objects]), 0, bb->num_alloc_objects - prev_num_alloc_objects);
  }
...
struct drm_i915_gem_exec_object2 *i915_batchbuf_add_buff(struct batchbuffer *bb, struct buff *buf, uint64_t alignment, bool write) {
  struct drm_i915_gem_exec_object2 *object;
...
struct drm_i915_gem_exec_object2 *i915_batchbuf_find_object(struct batchbuffer *bb, uint32_t handle) {
...
struct batchbuffer *i915_batchbuf_create(device_info *devinfo, uint32_t ctx, uint64_t size) {
  int ret;
  uint64_t start;
  struct batchbuffer *bb;
  struct drm_i915_gem_exec_object2 *object;
  
  start = i915_safe_offset_for_memory_regions(devinfo->fd, devinfo->memory_regions);
  ...
  if(ret != 0) {
    fprintf(stderr, "Failed to create a GEM while creating a batchbuffer.\n");
    return NULL;
  }
  bb->size = size;
  bb->ctx = ctx;
  ...
  object = i915_batchbuf_add_object(bb, bb->handle, bb->size, BUFF_INVALID_ADDRESS, bb->alignment, false);
...
static uint64_t i915_batchbuf_add_reloc(struct batchbuffer *bb,
                                   uint32_t to_handle, uint32_t handle, uint32_t read_domains, uint32_t write_domain,
                                   uint64_t delta, uint64_t offset, uint64_t presumed_offset) {
  struct drm_i915_gem_relocation_entry *relocs;
  ...
  if (to_handle == bb->handle) {
    ...
    if (bb->num_relocs == bb->allocated_relocs) {
      ...
      relocs = realloc(relocs, sizeof(*relocs) * bb->allocated_relocs);
      bb->relocs = relocs;
    }
    i = bb->num_relocs++;
  } else {
    ...
    relocs = (struct drm_i915_gem_relocation_entry *) to_object->relocs_ptr;
              ...
              uint64_t presumed_offset)
{
  ...
  address = i915_batchbuf_add_reloc(bb, to_handle, handle,
...
static struct drm_i915_gem_exec_object2 *i915_batchbuf_create_objects_array(struct batchbuffer *bb) {
  struct drm_i915_gem_exec_object2 *objects;
  uint32_t i;
...
static void i915_batchbuf_update_offsets(struct batchbuffer *bb, struct drm_i915_gem_exec_object2 *objects) {
  struct drm_i915_gem_exec_object2 *object;
  struct buff *entry;
  uint32_t i;
...
/*   intel_bb_dump_execbuf(bb, &execbuf); */
  ...
  if (ret) {
    free(objects);
    fprintf(stderr, "Failed to submit the execbuf from the batchbuffer.\n");
    return -1;
/*     intel_bb_dump_execbuf(bb, &execbuf); */
/*     free(objects); */
/*     return ret; */
