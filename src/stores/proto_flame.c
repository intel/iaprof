...
void store_cpu_side(uint64_t index, struct buffer_profile *gem)
{
        ...
        if (gem->execbuf_stack_str) {
                proto_flame_arr[index].cpu_stack = strdup(gem->execbuf_stack_str);
        } else {
...
void store_gpu_side(uint64_t index, char *stall_type,
                    uint64_t count, uint64_t addr,
                    uint64_t offset, char *insn_text)
{
...
void store_kernel_flames(struct buffer_profile *gem, int gem_index, char **insn_text, size_t *insn_text_len)
{
        int n;
	...
	hash_table_traverse(interval_profile_arr[gem_index].counts, offset, tmp) {
		...
		if ((*found)->active) {
			...
			store_cpu_side(index, gem);
                        store_gpu_side(index, "active", (*found)->active, addr, offset, tmp_insn_text);
		}
		if ((*found)->other) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "other", (*found)->other, addr, offset, tmp_insn_text);
		}
		if ((*found)->control) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "control", (*found)->control, addr, offset, tmp_insn_text);
		}
		if ((*found)->pipestall) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "pipestall", (*found)->pipestall, addr, offset, tmp_insn_text);
		}
		if ((*found)->send) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "send", (*found)->send, addr, offset, tmp_insn_text);
		}
		if ((*found)->dist_acc) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "dist_acc", (*found)->dist_acc, addr, offset, tmp_insn_text);
		}
		if ((*found)->sbid) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "sbid", (*found)->sbid, addr, offset, tmp_insn_text);
		}
		if ((*found)->sync) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "sync", (*found)->sync, addr, offset, tmp_insn_text);
		}
		if ((*found)->inst_fetch) {
                        index = grow_proto_flames();
			store_cpu_side(index, gem);
                        store_gpu_side(index, "inst_fetch", (*found)->inst_fetch, addr, offset, tmp_insn_text);
		}
	}
}
