...
static int __map_fd_by_name(char *name, int **fds)
{
	unsigned int id = 0;
	int fd, nb_fds = 0;
	void *tmp;
	...
	while (true) {
		...
		if (fd < 0) {
			fprintf(stderr, "can't get map by id (%u): %s",
			      id, strerror(errno));
			goto err_close_fds;
		}
		...
		if (err) {
			fprintf(stderr, "can't get map info (%u): %s",
			      id, strerror(errno));
			goto err_close_fd;
		}
...
int map_fd_by_name(char *name)
{
	...
	nb_fds = __map_fd_by_name(name, &fds);
	if (nb_fds != 1) {
		if (nb_fds > 1) {
			fprintf(stderr, "several maps match this handle");
			while (nb_fds--)
				close(fds[nb_fds]);
		}
		fd = -1;
