...
{
        int retval, i, config_id, fd, index;
        ...
        oa_units_info = NULL;
        retval = xe_query_oa_units(devinfo->fd, &oa_units_info);
        if (retval) {
                return retval;
        }
        if (!oa_units_info) {
        ...
        for (i = 0; i < oa_units_info->num_oa_units; i++) {
                ...
                if (oau->oa_unit_type == DRM_XE_OA_UNIT_TYPE_OAG) {
                        unit_id = oau->oa_unit_id;
                }
        }
        free(oa_units_info);
        if (unit_id == UINT_MAX) {
                fprintf(stderr, "Failed to find an OA unit of type DRM_XE_OA_UNIT_TYPE_OAG! Aborting.\n");
                return -1;
        }
