...
void collect_thread_interval(int s) {
}
...
int init_interval_signal() {
  ...
  sa.sa_flags = 0;
  sa.sa_handler = collect_thread_interval;
...
void collect_thread_stop(int s) {
...
void *collect_thread_main(void *a) {
  ...
  /* Make sure we get at least one interval; if
      ...
      manually */
  if(interval_num == 0) {
    pthread_kill(collect_thread_id, interval_signal);
  }
  
  collect_thread_stop(SIGTERM);
  return NULL;
}
...
int main(int argc, char **argv) {
  ...
  while(stopping == 0) {
    ...
    if(ret > 0) {
      printf("I read %d bytes something successfully!\n", ret);
    }
  }
  
  pthread_kill(collect_thread_id, SIGTERM);
