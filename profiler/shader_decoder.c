...
char *iga_disassemble_single(iga_context_t *ctx, unsigned char *data)
{
	...
	status = iga_disassemble_instruction(*ctx, &opts, data, NULL, NULL,
					     &text);
	if (status != IGA_SUCCESS) {
		fprintf(stderr,
			"IGA failed to disassemble an instruction! Error: %s\n",
			iga_status_to_str(status));
		return NULL;
	}
	if (!text) {
		fprintf(stderr,
			"IGA failed to disassemble an instruction by returning a NULL pointer!\n");
		return NULL;
	}
			    ...
			    size_t data_sz)
{
	...
	iga_disassemble_options_t opts = {
		sizeof(iga_disassemble_options_t),
		IGA_FORMATTING_OPTS_DEFAULT | IGA_FORMATTING_OPT_PRINT_PC |
	...
	status = iga_disassemble(*ctx, &opts, data, assumed_end, NULL, NULL,
				 &text);
	if ((status != IGA_SUCCESS) && (status != IGA_DECODE_ERROR)) {
		fprintf(stderr,
			"IGA failed to disassemble a kernel! Error: %s\n",
			iga_status_to_str(status));
	} else {
